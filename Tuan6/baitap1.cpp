...
void Display(Queue Q)
{
	while(Q.Front!=NULL)
	{
		cout<<Q.Front->Data.manv<<"\t"<<Q.Front->Data.hoten<<"\t"<<Q.Front->Data.heso<<"\t"<<Q.Front->Data.tienluong<<endl;
...
//Tìm và trả về tiền lương nhỏ nhất sử dụng hàm pop. Nếu Queue rỗng thì trả về -1
int Sellyear(Queue Q){
	Item Data;
	int min;
	if(isEmpty(Q))
	return -1;
...
//Dựa vào hàm Sellyear viết 1 hàm trả về nhân viên (Item) có nhiều tiền nhất)
	Item Oldest(Queue Q){
		Item Data, DataSell;
		int min;
		DataSell =Pop(Q);
		min=DataSell.tienluong;
		while (Q.Front!=NULL)
		{
			Data=Pop(Q);
			if(min>Data.tienluong){
	...
	do
	{
		...
		switch(LuaChon)
		{
			...
			case 5: //Pop lấy và trả về
			{
				...
				x = Pop(Q);
				//In ra x
				cout<<x.manv<<"\t"<<x.hoten<<"\t"<<x.heso<<"\t"<<x.tienluong<<endl;
				break;
			}
			...
			case 7://Tiền ít nhất 
			{
				...
				if(isEmpty(Q)){
					...
					min=Sellyear(Q);
					cout<<"Nguoi co it tien nhat la:"<<min<<endl;
				}
			}
			case 8://In nhan vien nhieu tien nhat
			{
					Item Data;
